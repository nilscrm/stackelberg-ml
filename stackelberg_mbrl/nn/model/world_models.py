...
import stackelberg_mbrl.utils as utils
...
class WorldModel(AWorldModel):
    ...
    def fit_dynamics(self, s, a, s_next, fit_mb_size, fit_epochs, max_steps=1e4):
        ...
        X = (s, a)
        Y = s_next
        return fit_tuple(self.dynamics_net, X, Y, self.dynamics_opt, self.dynamics_loss, fit_mb_size, fit_epochs, max_steps=max_steps)
    ...
    def fit_reward(self, s, a, s_next, r, fit_mb_size, fit_epochs, max_steps=1e4):
        ...
        X = (s, a, s_next)
        Y = r
        return fit_tuple(self.reward_net, X, Y, self.reward_opt, self.reward_loss, fit_mb_size, fit_epochs, max_steps=max_steps)
    ...
    def draw_mdp(self, filepath: Path, format: Literal["png", "svg"] = "png"):
        with torch.no_grad():
            ...
            rewards = [
                [[self.reward(s, a, s_next) for s_next in range(self.observation_dim)] for a in range(self.action_dim)]
                for s in range(self.observation_dim)
            ]
...
class StaticDiscreteModel(AWorldModel):
    ...
    def randomize(self):
        ...
        if self.reward_func is None:
            self.rewards = (torch.rand((self.state_dim, self.act_dim, self.state_dim)) + self.min_reward) * (
    ...
    def draw_mdp(self, filepath: Path, format: Literal["png", "svg"] = "png"):
        ...
        transition_probabilities = np.array(
            [[self.transition_probabilities[s][a].numpy() for a in range(self.action_dim)] for s in range(self.observation_dim)]
        )
...
class ContextualizedWorldModel(AWorldModel):
    ...
    def __init__(
        ...
        self.dynamics_net = MLP(
            input_dim=context_size + state_dim + act_dim, hidden_sizes=hidden_sizes, output_dim=state_dim, nonlinearity=activation
        )
        self.dynamics_opt = torch.optim.Adam(self.dynamics_net.parameters(), lr=fit_lr, weight_decay=fit_weight_decay)
        ...
        if rewards is None:
            self.rewards_net = MLP(
                input_dim=context_size + state_dim + act_dim + state_dim, hidden_sizes=hidden_sizes, output_dim=1, nonlinearity=activation
            )
    ...
    def fit_dynamics(self, observations, actions, observations_next, fit_mb_size, fit_epochs, max_steps=1e4):
        ...
        return fit(self.dynamics_net, X, Y, self.dynamics_opt, self.dynamics_loss, fit_mb_size, fit_epochs, max_steps=max_steps)
    ...
    def fit_reward(self, observations, actions, observations_next, rewards, fit_mb_size, fit_epochs, max_steps=1e4):
        ...
        Y = rewards
        return fit(self.rewards_net, X, Y, self.rewards_opt, self.rewards_loss, fit_mb_size, fit_epochs, max_steps=max_steps)

    def draw_mdp(self, context, filepath: Path, format: Literal["png", "svg"] = "png"):
        ...
        with torch.no_grad():
            ...
            for s in range(self.observation_dim):
                ...
                for a in range(self.action_dim):
                    action = one_hot(a, num_classes=self.act_dim).float()
                    state = one_hot(s, num_classes=self.observation_dim).float()
                    transition_probs[-1].append(self.next_state_distribution(np.concatenate((context, state)), action).numpy())

            rewards = [
                [
                    [
                        self.reward(torch.concat([context, one_hot(s, num_classes=self.observation_dim).float()]),
                                    one_hot(a, num_classes=self.action_dim).float(),
                                    torch.concat([context, one_hot(s_next, num_classes=self.observation_dim).float()])).item()
                        for s_next in range(self.observation_dim)
                    ]
                    for a in range(self.action_dim)
                ]
                for s in range(self.observation_dim)
