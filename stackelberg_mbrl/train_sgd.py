...
def train_contextualized_MAL():
    ...
    config = {
        ...
        "init_samples": 500,
        "policy_pretrain_steps": 1000,# TODO: make this sth large, like 1000,
    ...
    trainer = PPO("MlpPolicy", random_model_env, policy_kwargs=policy_kwargs, n_epochs=1)
    ...
    #trainer.load(path=config["pretrained_policy_save_file"])
    ...
    for iter in range(config["training_iterations"]):
        ...
        # states_ext = np.concatenate([states, s])
        # actions_ext = np.concatenate([actions, a])
        # next_states_ext = np.concatenate([next_states, s])
        states_ext = states
        actions_ext = actions
        next_states_ext = next_states
    
        dynamics_loss = model.fit_dynamics(states_ext, actions_ext, next_states_ext, 
