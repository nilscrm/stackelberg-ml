...
def train_contextualized_MAL(config: ExperimentConfig, draw: bool = False, verbose: bool = False, log: bool = False):
    ...
    match config.policy_config:
        case LoadPolicy():
            mprint("Loading policy model from file.")
        ...
        case PolicyConfig():
            ...
            mprint("Pretraining policy model")
            for iter in range(policy_config.pretrain_iterations):
                policy_ppo.learn(
                    policy_config.samples_per_training_iteration, 
                    tb_log_name="Policy" if log else None, 
                    reset_num_timesteps=(iter==0), 
                    progress_bar=verbose)

                mprint(f"Pretraining Iteration {iter}")
                with torch.no_grad():
                    for i, eval_random_env in enumerate(eval_envs):
                        ...
                        mprint(f"\tAvg Reward (random model {i}):      {eval_random_mean:.3f} ± {eval_random_std:.3f}")

                    eval_true_mean, eval_true_std = evaluate_policy(policy_ppo.policy, querying_env_true, n_eval_episodes=10)
                    mprint(f"\tAvg Reward (true env):      {eval_true_mean:.3f} ± {eval_true_std:.3f}")

                    eval_variant_mean, eval_variant_std = evaluate_policy(policy_ppo.policy, querying_env_eval, n_eval_episodes=10)
                    mprint(f"\tAvg Reward (eval env):   {eval_variant_mean:.3f} ± {eval_variant_std:.3f}")

                # TODO: how do we know we have converged? => we should do some sort of validation to see if we are still improving

            if policy_config.save_name is not None:
                policy_ppo.save(config.output_dir / config.experiment_name / "checkpoints" / policy_config.save_name)
        case _:
    ...
    class CountedPPOCallback(BaseCallback):
        ...
        def _on_step(self) -> bool: 
            ...
            return config.sample_efficiency.max_samples is None or self.samples < config.sample_efficiency.max_samples
    ...
    match config.world_model_config:
        ...
        case WorldModelConfig():
            ...
            kwargs = {}
            if config.world_model_config.ppo_kwargs is not None:
                kwargs.update(config.world_model_config.ppo_kwargs)
            model_ppo = PPO(
                "MlpPolicy",
                leader_env,
                tensorboard_log=config.output_dir / config.experiment_name / "tb" if log else None,
            ...
            mprint("Training world model")
            model_ppo.learn(
                total_timesteps=model_config.total_training_steps, 
                progress_bar=verbose, 
                tb_log_name="WorldModel" if log else None, 
                callback=callback)
...
if __name__ == "__main__":
    train_contextualized_MAL(poster_config, draw=True, verbose=True, log=True)
